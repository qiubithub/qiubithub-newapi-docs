...
::: tip 接口地址
Codex 是 OpenAI 兼容客户端，`base_url` 填 `https://www.qiubithub.com/v1`。这里的 `/v1` 只给 Codex / OpenAI 兼容工具用，不要照搬到 Claude Code 或 Gemini CLI。
:::
...
1. **先问 AI**：把当前报错、配置文件内容和本页截图一起发给豆包或 DeepSeek，让它帮你检查命令是否执行完整。
2. **联系客服**：把关键报错截图和你的系统信息整理好，联系人工支持继续排查。
